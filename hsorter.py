...
class HSorterWindow(Gtk.ApplicationWindow):
    ...
    def _build_media(self) -> None:
        ...
        self.images_view.connect("key-press-event", self.on_images_key_press)
        self.images_view.set_can_focus(True)
        self.images_view.connect("button-press-event", self.on_images_focus)
        ...
        image_buttons = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        add_image = Gtk.Button(label="Добавить файл")
        add_image.connect("clicked", lambda _b: self.add_image())
        add_image_clipboard = Gtk.Button(label="Из буфера")
        add_image_clipboard.connect("clicked", lambda _b: self.add_image_from_clipboard())
        image_buttons.pack_start(add_image, True, True, 0)
        image_buttons.pack_start(add_image_clipboard, True, True, 0)
        images_box.pack_start(image_buttons, False, False, 0)
    ...
    def add_image_from_clipboard(self) -> None:
        ...
        if not pixbuf:
            self._message("Буфер обмена", "В буфере обмена нет изображения.")
            return

        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S_%f")
        target_path = os.path.join(self._cache_dir(), f"clipboard_{timestamp}.png")
        try:
            pixbuf.savev(target_path, "png", [], [])
        except Exception as exc:
            self._message("Буфер обмена", f"Не удалось сохранить изображение из буфера: {exc}")
            return

        self.db.add_media(self.current_title_id, "image", target_path, "")
        self.refresh_media_lists()
    ...
    def on_images_focus(self, view, _event) -> bool:
        view.grab_focus()
        return False

    # Горячие клавиши для списка изображений (Ctrl+V = вставка из буфера).
    def on_images_key_press(self, _view, event) -> bool:
        if event.keyval == Gdk.KEY_v and (event.state & Gdk.ModifierType.CONTROL_MASK):
            self.add_image_from_clipboard()
            return True
        return False
    ...
    # Чтение настроек подключения к Emby из таблицы settings.
    # Возвращает словарь с параметрами API для проверки и экспорта.
    ...
    # Нормализация адреса Emby: добавляем http://, если схема не задана.
    # На выходе всегда URL без завершающего "/".
    ...
    # Проверка доступности Emby API и (опционально) корректности user_id.
    # Используется из окна настроек и перед запуском экспорта.
    ...
    # Валидация тегов во всей БД: нормализация, применение правил и обновление статусов
    # "отсутствуют теги"/"неполные теги" (по tagme) с последующей записью кеша статистики.
    ...
    # Экспорт тегов и провайдеров из локальной БД в Emby.
    # Алгоритм: получаем элементы Emby -> сопоставляем с локальными тайтлами
    # по именам/каталогам -> обновляем TagItems и ProviderIds (AniDB).
    ...
    # Путь к каталогу приложения (где лежит hsorter.py).
    def _app_dir(self) -> str:
        return os.path.dirname(__file__)

    # Каталог хранения резервных копий. Создаётся автоматически при обращении.
    def _backups_dir(self) -> str:
        path = os.path.join(self._app_dir(), ".backups")
        os.makedirs(path, exist_ok=True)
        return path

    # Полный путь к каталогу кеша изображений/миниатюр приложения.
    ...
    # Парсинг даты/времени из имени архива вида hsorter_backup_YYYYMMDD_HHMM.7z.
    # Возвращает datetime или None, если имя не соответствует формату.
    ...
    # Чтение списка резервных копий из .backups с сортировкой (сначала новые).
    # Для UI возвращаем путь, время создания (из имени) и размер файла.
    ...
    # Диалог резервного копирования: создание новых архивов и восстановление старых.
    # Показывает список копий с датой/размером и статус выполнения операций.
    ...
    def open_backup_dialog(self) -> None:
        ...
        def set_busy(busy: bool) -> None:
            create_button.set_sensitive(not busy)
            restore_button.set_sensitive((not busy) and bool(get_selected_path()))
        ...
        def run_long_task(status_text: str, task_fn):
            set_busy(True)
            progress.set_fraction(0.0)
            progress.set_text("Выполнение...")
        ...
        def on_create_clicked(_button) -> None:
            try:
                path = run_long_task(
                    ...
                    lambda: self._create_backup_archive(progress),
                )
                refresh_backup_list()
                set_status(f"Резервная копия создана: {os.path.basename(path)}")
            except Exception as exc:
        ...
        def on_restore_clicked(_button) -> None:
            ...
            try:
                run_long_task(
                    ...
                    lambda: self._restore_backup_archive(selected_path, progress),
                )
                set_status("Восстановление завершено.")
            except Exception as exc:
                set_status(f"Ошибка восстановления: {exc}")
